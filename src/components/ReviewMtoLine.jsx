...
function ReviewMtoLine({ mtolinelist }) {
  ...
  const handleEditOpen = (index) => {
    ...
    setEditedLineData(mtolinelist[index]);
  };
  ...
  const handleSave = (tag) => {
    const updatedLineList = [...mtolinelist];
    updatedLineList[editedRowIndex] = { ...editedLineData, tag: tag };

    setEditedRowIndex(-1);
    setEditedLineData({});

    window.api.send("update-mtolinelist-table", editedLineData);
  };
  ...
  const handleExport = () => {
    ...
    const dataToExport = mtolinelist.length > 0 ? mtolinelist : [];
  ...
  const filteredLineList = mtolinelist.filter(line =>
            ...
            <tbody>
              {filteredLineList.map((line, index) => (
                <tr key={index} style={{ color: 'black' }}>
                  ...
                  <td>{editedRowIndex === index ? <input onChange={e => handleChange('sysname', e.target.value)} type="text" value={editedLineData.sysname || ''} /> : line.sysname}</td>
